...
@section('title', 'Grup Teknisi')
@section('page-title', 'Manajemen Grup Teknisi')
...
<div class="container-fluid px-4 py-6 w-full mx-auto max-w-full">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        <div class="relative w-full xl:w-auto flex-1 max-w-md">
            {{-- Optional search form if needed later --}}
        </div>

        {{-- ACTION BUTTONS --}}
        <div class="flex flex-wrap sm:flex-nowrap gap-3 w-full xl:w-auto shrink-0">
            @if(!auth()->user()->isManajer())
            <a href="{{ route('admin.groups.create') }}" class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all shadow-sm focus:ring-2 focus:ring-blue-500 focus:ring-offset-1">
        ...
        <div class="w-full overflow-x-auto relative custom-scrollbar">
            <table class="min-w-max w-full text-sm text-left text-gray-600 border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="hover:bg-gray-50/80 transition-colors duration-150 group-row">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <span class="{{ $group->badge_class ?? 'bg-blue-100 text-blue-700 border-blue-200' }} px-3 py-1.5 rounded-full text-xs font-bold border inline-flex items-center gap-1.5">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-xs text-gray-500 truncate max-w-xs" title="{{ $group->description }}">
                                    {{ $group->description ?: '—' }}
                                </p>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.groups.edit', $group->id) }}" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-gray-200 text-gray-500 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-200 transition-all shadow-sm focus:outline-none" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    
                                    <button onclick="confirmDelete('{{ $group->id }}', '{{ $group->name }}')" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-gray-200 text-gray-500 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-sm focus:outline-none" title="Hapus Grup">
                                        <i class="fa-solid fa-trash-can"></i>
                                    </button>
                                    <form id="delete-form-{{ $group->id }}" action="{{ route('admin.groups.destroy', $group->id) }}" method="POST" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ !auth()->user()->isManajer() ? '6' : '5' }}" class="text-center py-16">
