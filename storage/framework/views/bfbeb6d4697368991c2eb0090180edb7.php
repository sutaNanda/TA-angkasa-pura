...
<?php $__env->startSection('title', 'Template Checklist'); ?>
<?php $__env->startSection('page-title', 'Manajemen Template SOP'); ?>
...
<?php $__env->startSection('content'); ?>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__empty_1 = true; $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $template): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 transition group template-row" 
                            data-name="<?php echo e(strtolower($template->name)); ?>" 
                            data-category="<?php echo e($template->category_id ?? 'umum'); ?>">
                            
                            <td class="px-6 py-4 text-center font-bold text-gray-400 text-xs">
                                <?php echo e(($templates->currentPage() - 1) * $templates->perPage() + $loop->iteration); ?>

                            </td>

                            <td class="px-6 py-4">
                                <div class="font-bold text-gray-800 text-base"><?php echo e($template->name); ?></div>
                                <div class="text-xs text-gray-500 mt-0.5"><?php echo e(Str::limit($template->description, 50) ?: 'Tidak ada deskripsi'); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($template->category): ?>
                                    <span class="inline-flex items-center gap-1 bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">
                                        <i class="<?php echo e($template->category->icon ?? 'fa-solid fa-box'); ?> text-gray-400"></i>
                                        <?php echo e($template->category->name); ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-700 font-bold text-xs border border-gray-200">
                                    <?php echo e($template->items->count()); ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <button onclick="showDetailSOP(<?php echo e($template->id); ?>)" class="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 hover:bg-blue-600 hover:text-white transition flex items-center justify-center" title="Lihat Detail">
                                        <i class="fa-solid fa-eye"></i>
                                    </button>

                                    <?php if(!auth()->user()->isManajer()): ?>
                                    <button onclick="editTemplate(<?php echo e($template->id); ?>)" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition flex items-center justify-center" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="deleteTemplate(<?php echo e($template->id); ?>)" class="w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition flex items-center justify-center" title="Hapus">
                        ...
                        <div class="w-full md:w-1/3 bg-gray-50 px-6 py-6 border-r border-gray-100 overflow-y-auto space-y-5">
                            ...
                            <div>
                                ...
                                <select id="category_id" name="category_id" class="w-full border-gray-300 rounded-lg text-sm bg-white border-2 border-gray-300 pl-2 py-2" required>
                                    ...
                                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($cat->id); ?>" class="text-gray-500"><?php echo e($cat->name); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
    ...
    <script>
        ...
        async function saveChecklist(e) {
            ...
            const url = id ? `/admin/checklists/${id}` : "<?php echo e(route('admin.checklists.store')); ?>";
            ...
            try {
                const response = await fetch(url, {
                    method: method, headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
        ...
        function deleteTemplate(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await fetch(`/admin/checklists/${id}`, { method: 'DELETE', headers: {'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'} });
                        if(response.ok) {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/admin/checklists/index.blade.php ENDPATH**/ ?>