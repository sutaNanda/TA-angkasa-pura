...
<?php $__env->startSection('page-title', 'Logbook Pengecekan Rutin'); ?>
...
<div class="container-fluid px-4 py-6 w-full mx-auto max-w-full">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 mb-6 w-full">
        <form method="GET" action="<?php echo e(route('admin.maintenances.index')); ?>" class="flex flex-col xl:flex-row gap-4 items-end">
            
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
                
                <div>
                    <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Dari Tanggal</label>
                    <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="w-full border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 px-3 py-2.5 outline-none shadow-sm transition-all text-gray-600">
                </div>
                
                
                <div>
                    <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Sampai Tanggal</label>
                    <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="w-full border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 px-3 py-2.5 outline-none shadow-sm transition-all text-gray-600">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="status" class="w-full appearance-none border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 pl-3 pr-10 py-2.5 outline-none shadow-sm transition-all bg-white text-gray-700">
                            ...
                            <option value="pass" <?php echo e(request('status') == 'pass' ? 'selected' : ''); ?>>Pass (Normal)</option>
                            <option value="fail" <?php echo e(request('status') == 'fail' ? 'selected' : ''); ?>>Fail (Ada Masalah)</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Aset / Teknisi / Tiket..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none shadow-sm transition-all text-gray-700">
                    </div>
                </div>
            </div>

            <div class="flex gap-2 w-full xl:w-auto shrink-0 pt-2 xl:pt-0">
                <a href="<?php echo e(route('admin.maintenances.index')); ?>" class="flex-1 xl:flex-none text-center bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm focus:ring-2 focus:ring-gray-200">
                    Reset
                </a>
            ...
            <table class="w-full min-w-[1100px] text-sm text-left text-gray-600 border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="<?php echo e($rowClass); ?> transition-colors duration-150">
                            <td class="px-6 py-4 text-center font-medium text-gray-400 text-xs whitespace-nowrap">
                                <?php echo e(($logs->currentPage() - 1) * $logs->perPage() + $loop->iteration); ?>

                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-bold text-gray-900 text-sm"><?php echo e($log->created_at ? $log->created_at->format('d M Y') : '-'); ?></div>
                                <div class="text-[11px] text-gray-500 mt-0.5 flex items-center gap-1.5"><i class="fa-regular fa-clock text-gray-400"></i> <?php echo e($log->created_at ? $log->created_at->format('H:i') . ' WITA' : '-'); ?></div>
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($log->asset): ?>
                                    <div class="font-bold text-gray-900 text-sm truncate max-w-[250px]" title="<?php echo e($log->asset->name); ?>"><?php echo e($log->asset->name); ?></div>
                                    <div class="text-[11px] text-gray-500 mt-0.5 font-mono">SN: <?php echo e($log->asset->serial_number ?? '-'); ?></div>
                                <?php elseif($log->location): ?>
                                    <div class="font-bold text-blue-800 text-sm flex items-center gap-1.5 truncate max-w-[250px]"><i class="fa-solid fa-layer-group text-blue-500"></i> <?php echo e($log->location->name); ?></div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($log->shift): ?>
                                    <span class="<?php echo e($log->shift->badge_class); ?> px-2.5 py-1 rounded-full text-[10px] font-bold border inline-flex items-center gap-1.5">
                                        <i class="<?php echo e($log->shift->icon_class); ?>"></i> <?php echo e($log->shift->name); ?>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap"><?php echo $findingBadge; ?></td>
                            <td class="px-6 py-4 text-center whitespace-nowrap"><?php echo $actionBadge; ?></td>

                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <button onclick="showDetailLog(<?php echo e($log->id); ?>)" class="text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 hover:text-blue-600 font-semibold text-xs px-4 py-1.5 rounded-lg transition w-full shadow-sm focus:ring-2 focus:ring-gray-200 focus:outline-none flex items-center justify-center gap-1.5">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/admin/maintenances/index.blade.php ENDPATH**/ ?>