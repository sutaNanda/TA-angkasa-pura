...
<?php $__env->startSection('page-title', 'Daftar Pengguna Sistem'); ?>
...
<div class="container-fluid px-4 py-6 w-full mx-auto max-w-full">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <form method="GET" action="<?php echo e(route('admin.users.index')); ?>" class="relative w-full xl:w-auto flex-1 max-w-md">
            <i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400"></i>
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama atau email..." class="pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-full text-sm shadow-sm transition-all text-gray-700">
        </form>
            ...
            <table class="min-w-max w-full text-sm text-left text-gray-600 border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="hover:bg-gray-50/80 transition-colors duration-150 group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3.5">
                                    <div class="w-10 h-10 rounded-full shrink-0 <?php echo e($user->role == 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'); ?> flex items-center justify-center font-bold text-xs shadow-sm overflow-hidden ring-2 ring-white">
                                        <?php if($user->avatar): ?>
                                    ...
                                    <div class="flex flex-col ml-3">
                                        <p class="font-bold text-gray-900 text-sm group-hover:text-blue-600 transition-colors"><?php echo e($user->name); ?></p>
                                        <p class="text-[11px] text-gray-500 mt-0.5"><?php echo e($user->email); ?></p>
                                    </div>
                                </div>
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ring-1 ring-inset <?php echo e($roleColor); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($user->group): ?>
                                    <span class="<?php echo e($user->group->badge_class ?? 'bg-blue-100 text-blue-700 border-blue-200'); ?> px-2.5 py-1 rounded-full text-[10px] font-bold border inline-flex items-center gap-1.5">
                                        <i class="fa-solid fa-users"></i> <?php echo e($user->group->name); ?>

                                    </span>
                                <?php elseif($user->role === 'user' && $user->department): ?>
                                    <span class="bg-gray-100 text-gray-700 border-gray-200 px-2.5 py-1 rounded-full text-[10px] font-bold border inline-flex items-center gap-1.5">
                                        <i class="fa-solid fa-building"></i> <?php echo e($user->department->name); ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button onclick="openEditModal(<?php echo e($user->id); ?>, '<?php echo e($user->name); ?>', '<?php echo e($user->email); ?>', '<?php echo e($user->role); ?>', '<?php echo e($user->technician_group_id); ?>', '<?php echo e($user->department_id); ?>')" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-gray-200 text-gray-500 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-200 transition-all shadow-sm focus:outline-none" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </button>
                                    
                                    
                                    <?php if(auth()->id() !== $user->id): ?>
                                        <button onclick="confirmDelete('<?php echo e($user->id); ?>')" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-gray-200 text-gray-500 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-sm focus:outline-none" title="Hapus/Nonaktifkan">
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr>
                            <td colspan="<?php echo e(!auth()->user()->isManajer() ? '7' : '6'); ?>" class="text-center py-16">
    ...
    <div id="exportProductivityModal" class="fixed inset-0 z-[60] hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                <form action="<?php echo e(route('admin.export.technician-productivity')); ?>" method="GET" target="_blank" onsubmit="setTimeout(() => closeModal('exportProductivityModal'), 500)">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/admin/users/index.blade.php ENDPATH**/ ?>