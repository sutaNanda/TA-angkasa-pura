...
<?php $__env->startSection('title', 'Tiket Perbaikan'); ?>
<?php $__env->startSection('page-title', 'Manajemen Tiket Perbaikan'); ?>
...
<div class="container-fluid px-4 py-6 w-full mx-auto">
    ...
    <div class="flex space-x-1 border-b border-gray-200 mb-6 overflow-x-auto custom-scrollbar pb-px">
        <a href="<?php echo e(route('admin.work-orders.index')); ?>" 
           class="flex items-center gap-2 px-5 py-3 font-semibold text-sm whitespace-nowrap border-b-2 transition-all duration-200 <?php echo e(!request('tab') ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-800 hover:border-gray-300'); ?>">
            <i class="fa-solid fa-layer-group <?php echo e(!request('tab') ? 'text-blue-500' : 'text-gray-400'); ?>"></i>
            Semua Tiket 
            <span class="py-0.5 px-2.5 rounded-full text-[11px] font-bold <?php echo e(!request('tab') ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'); ?>">
                <?php echo e($counts['all']); ?>

            </span>
        </a>
        <a href="<?php echo e(route('admin.work-orders.index', ['tab' => 'open'])); ?>" 
           class="flex items-center gap-2 px-5 py-3 font-semibold text-sm whitespace-nowrap border-b-2 transition-all duration-200 <?php echo e(request('tab') == 'open' ? 'text-red-600 border-red-600' : 'text-gray-500 border-transparent hover:text-gray-800 hover:border-gray-300'); ?>">
            <i class="fa-solid fa-envelope-open-text <?php echo e(request('tab') == 'open' ? 'text-red-500' : 'text-gray-400'); ?>"></i>
            Perlu Respon (Open)
            <span class="py-0.5 px-2.5 rounded-full text-[11px] font-bold <?php echo e(request('tab') == 'open' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'); ?>">
                <?php echo e($counts['open']); ?>

            </span>
        </a>
        <a href="<?php echo e(route('admin.work-orders.index', ['tab' => 'verify'])); ?>" 
           class="flex items-center gap-2 px-5 py-3 font-semibold text-sm whitespace-nowrap border-b-2 transition-all duration-200 <?php echo e(request('tab') == 'verify' ? 'text-yellow-600 border-yellow-500' : 'text-gray-500 border-transparent hover:text-gray-800 hover:border-gray-300'); ?>">
            <i class="fa-solid fa-clipboard-check <?php echo e(request('tab') == 'verify' ? 'text-yellow-500' : 'text-gray-400'); ?>"></i>
            Butuh Verifikasi
            <span class="py-0.5 px-2.5 rounded-full text-[11px] font-bold <?php echo e(request('tab') == 'verify' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'); ?>">
                <?php echo e($counts['verify']); ?>
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <form method="GET" class="flex flex-col md:flex-row items-start md:items-center gap-3 w-full xl:w-auto flex-1">
            <?php if(request('tab')): ?> <input type="hidden" name="tab" value="<?php echo e(request('tab')); ?>"> <?php endif; ?>
            ...
            <div class="relative w-1/2 md:max-w-xs shrink-0">
                ...
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari Tiket/Aset..." class="pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-full text-sm shadow-sm transition-all outline-none">
            </div>
            
            <div class="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-auto">
                <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="w-full sm:w-auto py-2.5 px-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm shadow-sm transition-all outline-none text-gray-600" title="Dari Tanggal">
                <span class="text-gray-400 font-medium hidden sm:block">-</span>
                <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="w-full sm:w-auto py-2.5 px-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm shadow-sm transition-all outline-none text-gray-600" title="Sampai Tanggal">
        ...
        <div class="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full xl:w-auto shrink-0">
            ...
            <a href="<?php echo e(route('admin.work-orders.export', request()->all())); ?>" target="_blank" class="w-full sm:w-auto bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm flex items-center justify-center gap-2 focus:ring-2 focus:ring-gray-200">
            ...
            <?php if(request('tab') == 'verify' && !auth()->user()->isManajer()): ?>
                <form action="<?php echo e(route('admin.work-orders.verify-all')); ?>" method="POST" onsubmit="return confirmVerifyAll(event)" class="w-full sm:w-auto">
                    <?php echo csrf_field(); ?>
                    <button type="submit" 
                            <?php echo e($counts['verify'] == 0 ? 'disabled' : ''); ?>
                            ...
                            class="w-full px-5 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 focus:ring-2 focus:ring-green-500 focus:ring-offset-1 whitespace-nowrap
                                <?php echo e($counts['verify'] > 0 ? 'bg-green-600 hover:bg-green-700 text-white shadow-sm hover:shadow animate-pulse' : 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed'); ?>">
                        <i class="fa-solid fa-check-double"></i> Verifikasi Semua (<?php echo e($counts['verify']); ?>)
                    </button>
                </form>
            <?php endif; ?>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-600 min-w-[1000px]">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $tickets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ticket): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="transition-colors duration-150 <?php echo e($rowHighlight); ?>">
                            <td class="px-6 py-4 whitespace-nowrap text-center font-medium text-gray-400 text-xs">
                                <?php echo e(($tickets->currentPage() - 1) * $tickets->perPage() + $loop->iteration); ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <div>
                                        <?php if($ticket->asset): ?>
                                            <div class="font-bold text-gray-900 text-sm truncate max-w-[200px]" title="<?php echo e($ticket->asset->name); ?>"><?php echo e($ticket->asset->name); ?></div>
                                            <div class="text-[11px] text-gray-500 mt-0.5 flex items-center gap-1.5">
                                                <i class="fa-solid fa-location-dot text-gray-400"></i> <span class="truncate max-w-[180px]"><?php echo e($ticket->asset->location->name ?? '-'); ?></span>
                                            </div>
                                        <?php else: ?>
                                            <div class="font-bold text-gray-900 text-sm truncate max-w-[200px]"><?php echo e($ticket->location->name ?? 'Lokasi tidak diketahui'); ?></div>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-gray-800 font-medium text-sm line-clamp-2 leading-relaxed" title="<?php echo e($ticket->issue_description); ?>">
                                    <?php echo e($ticket->issue_description); ?>

                                </div>
                                <div class="text-[11px] text-gray-400 mt-1.5 flex items-center gap-1.5">
                                    <?php if($ticket->maintenance_id): ?> 
                                    ...
                                    <?php endif; ?>
                                    <span class="whitespace-nowrap">&bull; <?php echo e($ticket->created_at->diffForHumans()); ?></span>
                                </div>
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ring-1 ring-inset <?php echo e($prioBadge); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold ring-1 ring-inset <?php echo e($statusConfig['bg']); ?> <?php echo e($statusConfig['text']); ?> <?php echo e($statusConfig['ring']); ?>">
                                    <i class="fa-solid <?php echo e($statusConfig['icon']); ?>"></i> <?php echo e($statusConfig['label']); ?>

                                </span>
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($ticket->executedBy || $ticket->technician): ?>
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-8 h-8 rounded-full bg-gray-800 text-white flex items-center justify-center text-xs font-bold shadow-sm shrink-0">
                                            <?php echo e(substr(($ticket->executedBy->name ?? $ticket->technician->name), 0, 1)); ?>

                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-xs font-bold text-gray-900 truncate max-w-[120px]"><?php echo e($ticket->executedBy->name ?? $ticket->technician->name); ?></span>
                                            <?php if($ticket->assignedGroup): ?>
                                                <span class="text-[10px] text-gray-500 truncate max-w-[120px]" title="<?php echo e($ticket->assignedGroup->name); ?>"><?php echo e($ticket->assignedGroup->name); ?></span>
                                            <?php else: ?>
                                                <span class="text-[10px] text-gray-500">Teknisi</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                <?php elseif($ticket->assignedGroup): ?>
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-gray-700 truncate max-w-[120px]"><i class="fa-solid fa-users text-gray-400 mr-1"></i> <?php echo e($ticket->assignedGroup->name); ?></span>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <?php if(!auth()->user()->isManajer() && ($ticket->status == 'open' || $ticket->status == 'handover')): ?>
                                    <button onclick="openAssignModal(<?php echo e($ticket->id); ?>, '<?php echo e($ticket->ticket_number); ?>', <?php echo e($ticket->technician_id ?? 'null'); ?>, '<?php echo e($ticket->priority); ?>')" class="bg-blue-600 text-white hover:bg-blue-700 px-4 py-1.5 rounded-lg text-xs font-bold transition shadow-sm w-full focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:outline-none">
                                        Tugaskan
                                    </button>
                                <?php elseif(!auth()->user()->isManajer() && $ticket->status == 'completed'): ?>
                                    <button onclick="openVerifyModal(<?php echo e($ticket->id); ?>, '<?php echo e($ticket->ticket_number); ?>')" class="bg-emerald-600 text-white hover:bg-emerald-700 px-4 py-1.5 rounded-lg text-xs font-bold transition shadow-sm w-full flex items-center justify-center gap-1.5 animate-pulse focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1 focus:outline-none">
                                        <i class="fa-solid fa-check-double"></i> Verifikasi
                                    </button>
                                <?php else: ?>
                                    <button onclick="showDetailModal(<?php echo e($ticket->id); ?>)" class="text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 hover:text-blue-600 font-semibold text-xs px-4 py-1.5 rounded-lg transition w-full shadow-sm focus:ring-2 focus:ring-gray-200 focus:outline-none">
                                        Lihat Detail
                                    </button>
                                <?php endif; ?>
                            </td>
    ...
    <div id="createModal" class="fixed inset-0 z-[60] hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                ...
                <form action="<?php echo e(route('admin.work-orders.store')); ?>" method="POST" class="px-6 py-5 space-y-5">
                    <?php echo csrf_field(); ?>
                    <div>
                        ...
                        <select name="asset_id" class="w-full border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 px-3 py-2.5 outline-none shadow-sm transition" required>
                            ...
                            <?php $__currentLoopData = $assets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $asset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($asset->id); ?>"><?php echo e($asset->name); ?> - <?php echo e($asset->location->name ?? 'Tanpa Lokasi'); ?></option>
            ...
            <div class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                ...
                <form id="assignForm" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <?php else: ?>
                            <p class="text-[11px] text-gray-500 mt-1.5"><?php echo e($technicians->count()); ?> teknisi tersedia</p>
                        <?php endif; ?>
                    </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/admin/work_orders/index.blade.php ENDPATH**/ ?>