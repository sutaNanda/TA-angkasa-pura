...
<?php $__env->startSection('title', 'Edit Rencana Perawatan'); ?>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                <a href="<?php echo e(route('admin.plans.index')); ?>" class="hover:text-blue-600 font-medium transition-colors">Rencana Perawatan</a>
                <i class="fa-solid fa-chevron-right text-[10px]"></i>
                <span class="text-gray-800 font-semibold">Edit Rencana</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                Edit Rencana 
                <span class="text-blue-600 px-3 py-1 bg-blue-50 border border-blue-100 rounded-lg text-lg line-clamp-1" title="<?php echo e($plan->name); ?>">
                    <?php echo e($plan->name); ?>

                </span>
            </h1>
            <p class="text-sm text-gray-500 mt-1">Sesuaikan konfigurasi target, aset, dan jadwal perawatan otomatis.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
            <a href="<?php echo e(route('admin.plans.index')); ?>" class="w-full md:w-auto text-center bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-5 py-2.5 rounded-lg text-sm font-medium transition shadow-sm focus:ring-2 focus:ring-gray-200 focus:outline-none">
                Batal
            </a>
            <button type="submit" form="editPlanForm" class="w-full md:w-auto inline-flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg text-sm font-semibold shadow-sm transition focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:outline-none">
                <i class="fa-solid fa-save"></i> Simpan Perubahan
            </button>
        </div>
    </div>

    <form action="<?php echo e(route('admin.plans.update', $plan->id)); ?>" method="POST" id="editPlanForm">
        <?php echo csrf_field(); ?>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-8">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="<?php echo e(old('name', $plan->name)); ?>" required placeholder="Contoh: Pengecekan Rutin" class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm outline-none">
                            <?php $__errorArgs = ['name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="text-red-500 text-xs mt-1.5 font-medium"><?php echo e($message); ?></p> <?php unset($message);
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <select :name="`configs[${index}][template_id]`" x-model="config.template_id" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm outline-none bg-white">
                                                ...
                                                <?php $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $template): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <option value="<?php echo e($template->id); ?>"><?php echo e($template->name); ?> (<?php echo e($template->frequency ?? '-'); ?>)</option>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </select>
                                        </div>
                    ...
                    <div class="p-6 space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <label class="cursor-pointer group">
                                    <input type="radio" name="frequency" value="<?php echo e($freq['value']); ?>" x-model="frequency" class="peer sr-only">
                                    <div class="p-4 rounded-xl border-2 transition-all duration-200 text-center bg-white border-gray-200 hover:bg-gray-50 hover:border-gray-300 peer-checked:border-<?php echo e($freq['color']); ?>-500 peer-checked:bg-<?php echo e($freq['color']); ?>-50/40 peer-checked:ring-1 peer-checked:ring-<?php echo e($freq['color']); ?>-500">
                                        <div class="w-10 h-10 mx-auto rounded-full bg-<?php echo e($freq['color']); ?>-100 text-<?php echo e($freq['color']); ?>-600 flex items-center justify-center mb-2.5 transition-transform duration-300 group-hover:scale-110">
                                            <i class="fa-solid <?php echo e($freq['icon']); ?> text-lg"></i>
                                        </div>
                                        <div class="font-bold text-gray-800 text-sm"><?php echo e($freq['title']); ?></div>
                                        <div class="text-[11px] text-gray-500 mt-0.5"><?php echo e($freq['desc']); ?></div>
                                    </div>
                                </label>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" value="<?php echo e(old('start_date', $plan->start_date ? $plan->start_date->format('Y-m-d') : date('Y-m-d'))); ?>" required class="w-full sm:w-1/2 rounded-xl border border-gray-300 bg-gray-50/50 px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all outline-none cursor-pointer text-gray-700" title="Tanggal Mulai">
                            ...
                            <div class="pt-6 border-t border-gray-100">
                                <?php echo $__env->make('admin.plans.partials.group_repeater', ['plan' => $plan], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                            </div>
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-6 lg:sticky lg:top-8">
                    ...
                    <div>
                        <div class="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 shadow-sm hover:border-blue-300 transition-colors cursor-pointer" @click="$refs.statusToggle.click()">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer pointer-events-none">
                                <input type="checkbox" x-ref="statusToggle" name="is_active" value="1" <?php echo e(old('is_active', $plan->is_active) ? 'checked' : ''); ?> class="sr-only peer">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="5" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm outline-none resize-none" placeholder="Tulis instruksi khusus atau catatan untuk teknisi di sini..."><?php echo e(old('notes', $plan->notes)); ?></textarea>
                    </div>
...
<script>
    function maintenancePlanForm() {
        return {
            targetType: '<?php echo e(old('target_type', $plan->target_type)); ?>',
            frequency: '<?php echo e(old('frequency', $plan->frequency)); ?>',
            configs: <?php echo json_encode(array_values(old('configs', $plan->template_configs ?? [['category_id' => '', 'template_id' => '']]))); ?>,
            assets: <?php echo json_encode($allCategoryAssets); ?>,
            selectedAssets: <?php echo json_encode(old('asset_ids', $plan->assets->pluck('id')->map(fn($id) => (string)$id)->toArray())); ?>,
            locations: <?php echo json_encode($locations); ?>,
            selectedLocations: <?php echo json_encode(old('location_ids', $plan->locations->pluck('id')->map(fn($id) => (string)$id)->toArray())); ?>,
            searchQueryAsset: '',
            searchQueryLocation: '',
            isLoading: false,
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/admin/plans/edit.blade.php ENDPATH**/ ?>