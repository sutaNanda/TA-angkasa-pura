...
<?php $__env->startSection('title', 'Riwayat Laporan Saya'); ?>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col items-start gap-3 hover:shadow-md transition-shadow duration-200 group">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-gray-900 leading-none"><?php echo e($tickets->total()); ?></p>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col items-start gap-3 hover:shadow-md transition-shadow duration-200 group">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-gray-900 leading-none"><?php echo e($tickets->where('status', 'open')->count()); ?></p> 
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col items-start gap-3 hover:shadow-md transition-shadow duration-200 group" title="Sedang dikerjakan oleh teknisi">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-gray-900 leading-none"><?php echo e($tickets->whereIn('status', ['in_progress', 'pending_part', 'handover'])->count()); ?></p> 
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col items-start gap-3 hover:shadow-md transition-shadow duration-200 group">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-gray-900 leading-none"><?php echo e($tickets->whereIn('status', ['completed', 'verified'])->count()); ?></p> 
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 flex flex-col lg:flex-row gap-6 relative overflow-hidden group">
                ...
                <div class="absolute left-0 top-0 bottom-0 w-1.5 
                    <?php echo e($ticket->status == 'open' ? 'bg-amber-400' : 
                ...
                <div class="flex items-start gap-4 lg:w-1/3 shrink-0">
                    <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-gray-50 border border-gray-200 flex-shrink-0 overflow-hidden flex items-center justify-center shadow-sm">
                        <?php if($ticket->asset && $ticket->asset->image): ?>
                            <img src="<?php echo e(asset('storage/'.$ticket->asset->image)); ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                            <i class="fa-solid <?php echo e($ticket->asset ? 'fa-box' : 'fa-layer-group'); ?> text-gray-300 text-2xl"></i>
                        <?php endif; ?>
                    </div>
                    <div class="flex flex-col min-w-0 pt-0.5">
                        <div class="flex items-center gap-2 mb-1.5">
                            <span class="text-[10px] font-bold font-mono text-gray-600 bg-gray-100 px-2 py-0.5 rounded-md border border-gray-200 tracking-wide"><?php echo e($ticket->ticket_number); ?></span>
                            <span class="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md border
                                <?php echo e($ticket->priority == 'high' ? 'bg-rose-50 text-rose-600 border-rose-200' : 
                        ...
                        <?php if($ticket->asset): ?>
                            <p class="font-bold text-gray-900 text-sm sm:text-base line-clamp-1" title="<?php echo e($ticket->asset->name); ?>"><?php echo e($ticket->asset->name); ?></p>
                            <p class="text-[11px] text-gray-500 mt-1 line-clamp-1 flex items-center gap-1.5">
                                <i class="fa-solid fa-location-dot text-gray-400"></i> <?php echo e($ticket->asset->location->name ?? '-'); ?>

                            </p>
                        <?php else: ?>
                            <p class="font-bold text-gray-900 text-sm sm:text-base line-clamp-1"><?php echo e($ticket->location->name ?? 'Lokasi tidak diketahui'); ?></p>
                ...
                <div class="lg:w-2/5 flex flex-col justify-center">
                    ...
                    <p class="font-medium text-gray-700 text-sm line-clamp-2 md:line-clamp-3 leading-relaxed" title="<?php echo e($ticket->issue_description); ?>">
                ...
                <div class="lg:w-1/4 flex flex-row lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-4 lg:gap-3 border-t lg:border-t-0 lg:border-l border-gray-100 pt-4 lg:pt-0 lg:pl-6 w-full shrink-0">
                    ...
                    <div class="flex flex-col lg:items-end gap-2">
                        ...
                        <?php elseif(in_array($ticket->status, ['in_progress', 'pending_part', 'handover'])): ?>
                        ...
                        <?php elseif(in_array($ticket->status, ['completed', 'verified'])): ?>
                        ...
                        <div class="text-left lg:text-right">
                            <p class="text-[11px] font-bold text-gray-600"><?php echo e($ticket->created_at->format('d M Y')); ?></p>
                            <p class="text-[10px] text-gray-400 mt-0.5"><i class="fa-regular fa-clock"></i> <?php echo e($ticket->created_at->format('H:i')); ?> WITA</p>
                        </div>
                    </div>
...
<?php echo $__env->make('layouts.user', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/user/tickets/index.blade.php ENDPATH**/ ?>