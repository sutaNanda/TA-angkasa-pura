    ...
    <div x-data="{ showScanOptions: false }" class="flex flex-col min-h-screen md:flex-row md:h-screen md:overflow-hidden relative">
        ...
        <aside class="hidden md:flex flex-col w-64 bg-slate-900 text-white flex-shrink-0 h-screen sticky top-0 overflow-hidden shadow-2xl z-50">
            ...
            <nav class="flex-1 p-4 space-y-1.5 overflow-y-auto">
                ...
                <a href="<?php echo e(route('technician.dashboard')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-all duration-200 group <?php echo e(request()->routeIs('technician.dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'); ?>">
                ...
                <a href="<?php echo e(route('technician.tasks.index')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-all duration-200 group <?php echo e(request()->routeIs('technician.tasks.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'); ?>">
                ...
                <a href="<?php echo e(route('technician.history.index')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-all duration-200 group <?php echo e(request()->routeIs('technician.history.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'); ?>">
                ...
                <a href="<?php echo e(route('technician.profile.index')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-all duration-200 group <?php echo e(request()->routeIs('technician.profile.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'); ?>">
                ...
                <a href="<?php echo e(route('technician.assets.index')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-all duration-200 group <?php echo e(request()->routeIs('technician.assets.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'); ?>">
        ...
        <div class="flex-1 flex flex-col min-w-0 bg-gray-50 overflow-hidden">
            ...
            <header class="bg-blue-600 text-white px-4 py-3 sticky top-0 z-30 shadow-md md:hidden flex justify-between items-center">
                <div class="flex flex-col">
                    ...
                    <h2 class="text-sm font-bold truncate"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h2>
                </div>
                <a href="<?php echo e(route('technician.profile.index')); ?>" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
                    <div class="text-right">
                        <p class="text-[10px] text-blue-200 font-bold leading-none mb-1">Halo</p>
                        <p class="text-xs font-black truncate max-w-[100px]"><?php echo e(auth()->user()->name); ?></p>
                    </div>
                    <?php if(auth()->user()->avatar): ?>
                        <img src="<?php echo e(asset('storage/' . auth()->user()->avatar)); ?>" class="w-8 h-8 rounded-full object-cover border border-white/50 shadow-sm">
                    <?php else: ?>
            ...
            <header class="hidden md:flex items-center justify-between px-8 py-4 bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40">
                <div class="flex items-center gap-4">
                    <h2 class="text-xl font-bold text-gray-800 tracking-tight">Halaman <?php echo $__env->yieldContent('title', 'Dashboard'); ?></h2>
                </div>
                
                <div class="flex items-center gap-6">
                    <a href="<?php echo e(route('technician.profile.index')); ?>" class="flex items-center gap-3 pl-6 border-l border-gray-200 hover:opacity-80 transition-opacity">
                        <div class="text-right">
                            <p class="text-sm font-bold text-gray-900 leading-none mb-1">Hallo, <?php echo e(auth()->user()->name); ?></p>
                            <p class="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Teknisi Lapangan</p>
                        </div>
                        <?php if(auth()->user()->avatar): ?>
                            <img src="<?php echo e(asset('storage/' . auth()->user()->avatar)); ?>" class="w-10 h-10 rounded-full object-cover border border-gray-200 shadow-sm">
                        <?php else: ?>
            ...
            <div class="md:hidden">
                <?php echo $__env->make('components.technician-bottom-nav', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
        ...
        <div x-cloak x-show="showScanOptions" style="z-index: 9999;" class="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4" x-transition.opacity>
            <div class="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-fadeIn" @click.away="showScanOptions = false">
                ...
                <div class="p-6 flex flex-col gap-3">
                    <a href="<?php echo e(route('technician.scan')); ?>" class="flex items-center gap-4 bg-blue-600 hover:bg-blue-700 text-white px-5 py-4 rounded-2xl font-bold shadow-md shadow-blue-600/20 transition group">
            ...
            if(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if(code) {
                        ...
                        fetch("<?php echo e(route('technician.scan.process')); ?>", {
                            method: "POST",
                            headers: {
...
<?php /**PATH C:\Users\User\Documents\tugas kuliah\TA\asset-monitoring\resources\views/layouts/technician.blade.php ENDPATH**/ ?>